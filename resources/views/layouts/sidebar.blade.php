    ...
    <div id="sidebar" class="offcanvas offcanvas-start overflow-y-auto" data-bs-theme="dark" data-bs-backdrop="true" tabindex="-1" aria-labelledby="sidebarLabel">
        <nav class="d-flex flex-column">
            <div class="offcanvas-body">
                <!-- サイドバーのトグルボタン -->
                <button data-bs-dismiss="offcanvas" class="border rounded-3 p-1 text-decoration-none mb-3"><i class="bi bi-caret-left-fill"></i> Menu</button>
                <div id="sidebar-nav" class="list-group border-0 rounded-0 text-sm-start " style="max-height: 80vh;">
                    @foreach ($latestArticles as $article)
                        <a href="{{ route('articles.show', $article->id) }}" class="list-group-item border-end-0 d-inline-block text-truncate bg-dark bg-gradient @if(request()->is('articles/' . $article->id)) active list-group-item-danger text-primary @endif" data-bs-parent="#sidebar">{{ $article->title }}</a>
                    @endforeach
                </div>
            </div>
